...
export class APIManager {
	...
	private constructor() {
		// this.BASE_API_URL = import.meta.env.VITE_BASE_API_URL as string;
		this.BASE_API_URL = "";
	}
	...
	public async get(url: string, config?: IMetadataNoBody) {
		...
		return await fetch(`${this.BASE_API_URL}${url}`, {
			headers,
		}).then((data) => data.json());
	}
	...
	public async post(url: string, config?: IMetadataWBody) {
		...
		return await fetch(`${this.BASE_API_URL}${url}${query}`, options).then((res) =>
			res.json()
		);
	}
